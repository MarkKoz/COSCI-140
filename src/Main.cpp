...
int main() {
	...
	try {
		...
		// Assign values to the radii of the circles.
		for (int x = 0; x < aVector[0].getCircleCount(); x++) {
			aVector[x].setCircleRadius(x + 1);
			bVector[x].setCircleRadius(x + 1.1);
		}

		cout << "There are " << aVector[0].getCircleCount()
		     << " circles with the following integer value radii:" << endl;

		for (int x = 0; x < aVector[0].getCircleCount(); x++) {
			cout << "Circle " << x + 1 << ": " << aVector[x].getCircleRadius()
			     << endl;
		}

		cout << "There are " << aVector[0].getCircleCount()
		     << " circles with the following decimal value radii:" << endl;

		for (int x = 0; x < aVector[0].getCircleCount(); x++) {
			cout << "Circle " << x + 1 << ": " << bVector[x].getCircleRadius()
			     << endl;
		}

	} catch (ArrayOfObjects<Circle<int>>::ErrorAllocatingPointerMemory e) {
		cout << "You entered the following value for the number of circles "
		     << e.getValue() << endl;
	...
	try {
		...
		cout << "There are " << cVector[0].getRectangleCount()
		     << " rectangles with the following integer value dimensions:"
		     << endl;

		for (int x = 0; x < cVector[0].getRectangleCount(); x++) {
			cout << "Rectangle " << x + 1 << ": width = "
			     << cVector[x].getRectangleWidth() << endl;
			cout << "Rectangle " << x + 1 << ": length = "
			     << cVector[x].getRectangleLength() << endl;
		}

		cout << "There are " << cVector[0].getRectangleCount()
		     << " rectangles with the following decimal value dimensions:"
		     << endl;

		for (int x = 0; x < dVector[0].getRectangleCount(); x++) {
			cout << "Rectangle " << x + 1 << ": width = "
			     << dVector[x].getRectangleWidth() << endl;
			cout << "Rectangle " << x + 1 << ": length = "
			     << dVector[x].getRectangleLength() << endl;
		}

		cout << "There are " << cVector[0].getRectangleCount()
		     << " rectangles with the following \"int-decimal\" value dimensions:"
		     << endl;

		for (int x = 0; x < dVector[0].getRectangleCount(); x++) {
			cout << "Rectangle " << x + 1 << ": width = "
			     << cdVector[x].getRectangleWidth() << endl;
			cout << "Rectangle " << x + 1 << ": length = "
			     << cdVector[x].getRectangleLength() << endl;
		}

	} catch (ArrayOfObjects<Rectangle<int, int>>::ErrorAllocatingPointerMemory e) {
		cout << "You entered the following value for the number of rectangles "
		     << e.getValue() << endl;
		cout << "you want created; this is an illegal value " << endl;
		cout << "and the vector was not created, but the rest of the " << endl;
		cout << "program continues... " << endl << endl;
	...
	try {
		...
		cout << "You entered the following value for the radius "
		     << e.getValue() << endl;
	...
	try {
		...
		cout << "You entered the following value for the rectangle "
		     << e.getValue() << endl;
		cout << "This is an illegal value for one of the rectangle dimensions"
		     << endl;
		cout << "and the rectangle was not created, but the rest of the "
		     << endl;
		cout << "program continues... " << endl;
	}
